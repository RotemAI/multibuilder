...
_SSH_FORWARDED_NAMES = frozenset(
    name for name in dir(ssh_service)
    if name.startswith("_ssh") or name.startswith("SSH_") or name in {
        "LOCAL_CONNECTION_KIND",
    }
)
...
_BROWSER_FORWARDED_NAMES = frozenset(
    name for name in dir(browser_service)
    if name.startswith("_browser") or name.startswith("BROWSER_") or name in {
        "PUBLIC_BASE_URL", "ROOT_PATH",
    }
)
